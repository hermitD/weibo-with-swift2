...
public class WBStatusLayout {
    ...
    func layoutTag() {
        ...
        if tag!.tagType == 1{
            ...
            text.setColor(kWBCellTextHighlightColor, range: NSMakeRange(0, text.length))
        ...
        if pageInfo!.type == 11 && pageInfo!.objectType == "video" {    // 视频卡片
            ...
            */
            ...
            if text.length > 0 {
                ...
                text.lineBreakMode = .ByTruncatingTail   //  ---->  xxx...
    ...
    func textWithStatus(status:WBStatus?, isRetweet: Bool, fontSize: CGFloat, textColor: UIColor) -> NSMutableAttributedString? {
        ...
        for wburl: WBURL in self.status.urlStruct {
            ...
            repeat {
                ...
                if text.attribute(YYTextHighlightAttributeName, atIndex: UInt(range.location)) == nil {
        ...
        for topic: WBTopic in self.status.topicStruct {
            ...
            let topicTitle: String! = "#\(topic.topicTitle)#"       // 两边添加#号  "XXX" -> "#XXX#"
        ...
        for emo: NSTextCheckingResult in emotionResults {
            ...
            let emoText: NSAttributedString = NSAttributedString.attachmentStringWithEmojiImage(image, fontSize: fontSize)
    ...
    func attachment(fontSize: CGFloat, imageURL: String, shrink: Bool) -> NSAttributedString {
        ...
        art.setTextAttachment(attachment, range: NSMakeRange(0, art.length))
        var ctDelegate = delegate.CTRunDelegate()
        art.setRunDelegate(ctDelegate, range: NSMakeRange(0, art.length))
